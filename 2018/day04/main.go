...
var (
	regex  = regexp.MustCompile(`\[(\d\d\d\d-\d\d-\d\d \d\d:\d\d)\] (.*)`)
	gregex = regexp.MustCompile(`Guard #(\d+) begins shift`)
	tmfmt  = `2006-01-02 15:04`
)
...
func Do(input string) {
	...
	for _, line := range lines {
		parts := regex.FindStringSubmatch(line)[1:]
		t, _ := time.Parse(tmfmt, parts[0])
		if gregex.MatchString(parts[1]) {
			id := gregex.FindStringSubmatch(parts[1])
			current, _ = strconv.Atoi(id[1])
			if g, ok := guards[current]; ok {
				guard = g
			} else {
				guards[current] = Guard{ID: current}
		...
		case "falls asleep":
			sleep = t
		case "wakes up":
			length := int(t.Sub(sleep).Minutes())
			guard.Sleeps = append(
				guard.Sleeps,
				Sleep{
					Day:    sleep,
	...
	for _, g := range guards {
		if g.Length > longest.Length {
			longest = g
		}
	}
	a, b := longest.MinuteSleep()
	fmt.Println(a * b)
}
func Do2(input string) {
	...
	for _, g := range guards {
		a, b := g.MinuteSleep()
		fmt.Println("G: ", a, "Min: ", b)
		if b > cnt {
			longest = g
			cnt = b
		}
	}
	a, _ := longest.MinuteSleep()
	fmt.Println(a * longest.ID)
}
...
func (g Guard) MinuteSleep() (int, int) {
	minutes := make([]int, 60)
	for _, sleep := range g.Sleeps {
		for i := 0; i < sleep.Length; i++ {
			t := sleep.Day.Add(time.Duration(i) * time.Minute)
			minutes[t.Minute()]++
		}
	}
	var mn, mx int
	for i, cnt := range minutes {
		if cnt > mx {
			mn = i
			mx = cnt
		}
...
var demo = `[1518-11-01 00:00] Guard #10 begins shift
...
var input = `[1518-04-22 00:52] wakes up
