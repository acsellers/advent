...
var (
	ops = map[string]func(string, int) bool{
		">": func(reg string, val int) bool {
...
func Evaluation(input string) (int, int) {
	...
	for scanner.Scan() {
		...
		// a inc -5 if a  > 1
		// %s %s %d if %s %s %d
		var dreg, chg, creg, op string
		var diff, cmp int
		fmt.Sscanf(scanner.Text(), "%s %s %d if %s %s %d", &dreg, &chg, &diff, &creg, &op, &cmp)
		//fmt.Println(dreg, chg, diff, "if", creg, op, cmp)
		if ops[op](creg, cmp) {
			switch chg {
			case "inc":
				registers[dreg] += diff
			...
			if registers[dreg] > largest {
				largest = registers[dreg]
			}
		}
	}
	//var largestk string
	...
	//fmt.Println(largestk, largestv)
	return largestv, largest
}

func main() {
	fmt.Println(Evaluation(TestInput))
...
var input = `yxr inc 119 if nev != 6
piw dec -346 if tl != 4
