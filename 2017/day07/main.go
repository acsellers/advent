...
func FindBottom(input string) string {
	...
	for scanner.Scan() {
		...
		var trailers []string
		var weight int
		fmt.Sscanf(scanner.Text(), "%s (%d) -> %v", &name, &weight, &lower)
		if lower != "" {
			trailers = strings.Split(strings.TrimSpace(strings.Split(scanner.Text(), "->")[1]), ", ")
		}
		nodes[name] = &Node{Name: name, Weight: weight, childs: trailers}
		names = append(names, name)
	}
	for _, name := range names {
		if len(nodes[name].childs) > 0 {
			for _, nn := range nodes[name].childs {
				nodes[name].Children = append(nodes[name].Children, nodes[nn])
				nodes[nn].Parent = nodes[name]
			}
		}
	}
	for _, n := range nodes {
...
func FindIncorrect(input string) (string, int) {
	nodes := make(map[string]*Node)
	names := []string{}
	scanner := bufio.NewScanner(strings.NewReader(input))
	for scanner.Scan() {
	...
	problem := root
	for {
		problemIx, norm := notequal(problem.Sums())
		problem = problem.Children[problemIx]
		nextIx, _ := notequal(problem.Sums())
		if nextIx == -1 {
			return problem.Name, problem.Weight + norm - problem.Sum()
		}
	}
	return "asdf", 0
}
func main() {
	fmt.Println("Bottom Test: ", FindBottom(testInput))
	fmt.Println("Bottom: ", FindBottom(input))
	n, d := FindIncorrect(testInput)
	fmt.Println("Bottom Bad: ", n, d)
	n, d = FindIncorrect(input)
	fmt.Println("Bottom Real Bad: ", n, d)
}
